...
FROM products
WHERE discontinued = 1 -- or WHERE discontinued IS TRUE
ORDER BY product_id;
--Here is we present True as 1 and False as 0 in SQL
...
---Step by step, we will add the clauses to filter the data.
--Step 1: Select product_name from products
SELECT product_name
FROM products;
--Step 2: Add WHERE clause to filter by units_in_stock and reorder_level
SELECT product_name
FROM products
WHERE units_in_stock <= reorder_level;
--Step 3: Add condition for discontinued products
SELECT product_name
FROM products
WHERE units_in_stock <= reorder_level
  AND discontinued = 0;
--Step 4: Add condition for units_on_order
SELECT product_name
FROM products
WHERE units_in_stock <= reorder_level
  AND discontinued = 0
  AND units_on_order > 0;
--Step 5: Order the results by product_id
SELECT product_name
FROM products
WHERE units_in_stock <= reorder_level
  AND discontinued = 0
...
--Combination of AND and OR is used to filter the data
SELECT product_name
FROM products
WHERE units_in_stock <= reorder_level
  AND discontinued = 0
...
--A typical workflow for these kinds of “day-to-day” queries is:
--Decide what you want to measure (count, sum, average, etc.).
--Decide how you want to group the data (by category, region, etc.).
--Add filters with WHERE (before grouping) and HAVING (after grouping).
--Order the results so the output is easier to interpret.
--Example — finding how many products are in each category:
...
-- Explanation:
--GROUP BY customer_id groups rows by each customer.
--COUNT(order_id) counts how many orders are in each group (it ignores NULL order_id values).
--ORDER BY order_count DESC, customer_id sorts by the customers with the most orders first; ties are broken by customer_id.

--If you want to include customers who have 0 orders (they won’t appear in the query above), use a LEFT JOIN against the customers table:
...
SELECT customer_id,ship_address,
COUNT(*) AS order_count
FROM orders
GROUP BY customer_id, ship_address
...
--If you wanted to sort by biggest spender first, you could change the last line to:
